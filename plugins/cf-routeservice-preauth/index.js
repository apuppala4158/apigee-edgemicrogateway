'use strict'
var debug = require('debug')('plugin:cf-routeservice-preauth');
...
function retarget (req, res, next) {
  ...
  if (cfHostname && !pathOnly) {
	...
	// set req.reqUrl.path to be updated with cf hostname to match the edge proxy basepaths
	req.reqUrl.path = '/' + cfHostname + cfPath;
	
  debug('new targetHostname: '+req.targetHostname);
  debug('new targetPath: ' + req.targetPath);
  debug('new requrlPath: ' + req.reqUrl.path);
